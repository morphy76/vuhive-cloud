...
func (s *SuiteCommand) Execute(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fprintln(stderr, "Usage: vuhive suite <upload> [flags] <path>")
		return 1
	}
	...
	default:
		fprintf(stderr, "Unknown suite subcommand: %s\n", args[0])
		return 1
	}
}
...
func (s *SuiteCommand) upload(args []string, stdout, stderr io.Writer) int {
	...
	positional := fs.Args()
	if len(positional) == 0 {
		fprintln(stderr, "Error: missing source archive or directory path")
		return 1
	}
	...
	if suiteID == "" {
		fprintln(stderr, "Error: --suite-id is required")
		return 1
	}
	...
	if creds != nil && creds.AccessToken != "" {
		if claims, err := creds.Claims(); err == nil && claims != nil {
			if !claims.HasRole(model.RoleDeveloper) && !claims.HasRole(model.RoleAdmin) {
				fprintf(stderr, "Error: role guard violation: user %q lacks required 'vuhive-developer' role\n", claims.Username())
				return 1
			}
		}
	}
	...
	if err != nil {
		fprintf(stderr, "Error accessing source path %q: %v\n", sourcePath, err)
		return 1
	}
	...
	if fi.IsDir() {
		...
		if err != nil {
			fprintf(stderr, "Error packaging source directory: %v\n", err)
			return 1
		}
		tarData = data
	} else {
		...
		if err != nil {
			fprintf(stderr, "Error reading source archive: %v\n", err)
			return 1
		}
		tarData = data
	}
	...
	if err != nil {
		fprintf(stderr, "Error creating multipart file: %v\n", err)
		return 1
	}
	if _, err := part.Write(tarData); err != nil {
		fprintf(stderr, "Error writing multipart data: %v\n", err)
		return 1
	}
	...
	if err != nil {
		fprintf(stderr, "Error creating upload request: %v\n", err)
		return 1
	}
	...
	if err != nil {
		fprintf(stderr, "Upload failed: %v\n", err)
		return 1
	}
	...
	if resp.StatusCode != http.StatusAccepted && resp.StatusCode != http.StatusOK {
		fprintf(stderr, "Build initiation failed (HTTP %d): %s\n", resp.StatusCode, string(respBody))
		return 1
	}
	...
	fprintln(stdout, "=== Source Uploaded & Compilation Initiated ===")
	fprintf(stdout, "Message:   %s\n", result.Message)
	for _, a := range result.Artifacts {
		fprintf(stdout, "Artifact:  %s (Platform: %s, Status: %s)\n", a.ID, a.Platform, a.Status)
	}
