...
- 🛑 **Execution Lifecycle Control, Graceful Abort & Cleanup**: Real-time run monitoring, on-demand abort (`POST /api/v1/runs/{id}/abort`), safe runtime resource cleanup (`POST /api/v1/runs/{id}/cleanup`), and permanent run deletion (`DELETE /api/v1/runs/{id}`) that instantly tears down Kubernetes workloads, reclaims cluster resources, or purges historical S3 logs and database records.
...
- 🔐 **Secure Storage Credential Injection**: Runner workloads inject object storage credentials via Kubernetes Secret references (`valueFrom.secretKeyRef` or `runner.existingSecret`) rather than plaintext environment variables, eliminating credential exposure via `kubectl get pod -o yaml`, with native fallback to IAM Roles for Service Accounts (IRSA) and Workload Identity.
...
- 🧩 **Reusable Runner Profiles & Decoupled Init Sizing**: Decouples scenario code from scheduling constraints with reusable profiles defining CPU/RAM requests and limits (Guaranteed QoS), node selectors, tolerations, node affinities, and kernel-isolated container sandboxes (e.g. gVisor `runsc`, Kata Containers). The `runner-init` bootstrap container resources are independently parameterized via Helm (`runner.initResources`) to prevent cluster over-allocation.
- ⚡ **Go Runtime Container Ergonomics & Rapid Readiness**: All Go container entrypoints automatically tune `GOMAXPROCS` to container CFS quotas via `go.uber.org/automaxprocs` (with debug-level suppression in init containers) to eliminate CPU throttling spikes, enforce `GOMEMLIMIT` soft memory boundaries (85% of cgroup ceiling) to prevent `OOMKilled` terminations before GC triggers, and incorporate rapid `startupProbe` polling for sub-second cold-start pod readiness.
...
- 🌐 **Modern React 19 PWA Web Dashboard**: Progressive Web App frontend (`web/`) with dark/light themes, offline query cache persistence via IndexedDB, automated retry backoff, circuit-breaker awareness with degraded service notifications, and live status updates, packaged with zero overhead into the Go BFF via Go `embed.FS`.
- 📜 **Interactive Telemetry, Configuration & Secrets Management**: Rich operator tools including virtualized ANSI log streaming (`run.log`), interactive tabular and collapsible JSON tree summary report inspectors (`summary.json`), YAML configuration editor with visual diffing and `${secrets.KEY}` placeholder insertion, and suite-scoped encrypted secrets management with AES-256-GCM encryption at rest (activated via `SECRETS_ENCRYPTION_KEY`).
- ⚡ **Go Backend-For-Frontend (cmd/bff) & Real-Time SSE Hub**: High-throughput gateway providing sub-50ms composite dashboard aggregation, persistent Server-Sent Events (SSE) telemetry fan-out (`run_status_changed`, `build_status_changed`), transparent reverse proxying, and outbound circuit breaker protection (`gobreaker`) short-circuiting downstream calls to prevent cascading failures during upstream outages.
