...
- 🌐 **Modern React 19 Web Interface & Progressive Web App (PWA)**: The official frontend (`web/`) built with React 19, Vite 6, TypeScript, and zero-runtime Tailwind CSS v4. Delivers a complete, installable Progressive Web App (PWA) configured via `vite-plugin-pwa` with `manifest.webmanifest`, high-resolution SVG and PNG app icons (192x192, 512x512, maskable), and Workbox service worker caching. Static JS, CSS, and font assets use a **Cache-First** strategy with 30-day retention, while API read endpoints (`/api/bff/v1/*`, `/api/v1/*`) leverage a **Network-First** strategy with 3-second network timeout and 24-hour cache fallback. Features an **Offline Read-Only Shell** powered by **TanStack Query** query cache persistence to IndexedDB via `idb-keyval`, an automatic non-intrusive offline indicator banner, and contextual **"Offline Preview"** tags on cached test suites and historical execution metrics during network instability. Includes an accessible **App Installation Prompt** (`beforeinstallprompt`) allowing operators to install the dashboard directly to desktop and mobile home screens, alongside WCAG 2.1 AA accessibility, keyboard navigation, dark/light themes, vendor-split chunking (`vendor-react`, `vendor-query`, `vendor-ui`, `vendor-radix`, `vendor-markdown`), automated **axe-core** CI audits, and zero-overhead binary packaging via the Go BFF's embedded `embed.FS` file server with immutable HTTP caching.
...
- 🧩 **Visual Runner Profile Builder for Resources, Affinities & Tolerations**: Interactive visual builder in the React 19 web dashboard for configuring Kubernetes compute environments and pod scheduling rules (`POST /api/v1/profiles` and `PUT /api/v1/profiles/{id}`). Features dual tactile range sliders and synchronized numeric inputs for CPU request/limit (`1000m` / `1 vCPU`) and RAM request/limit (`1Gi` / `2Gi`), automatic Guaranteed QoS detection, and real-time warning indicators when requests exceed limits. Includes a dynamic scheduling rules builder for Node Selectors (key-value labels), Node Affinities (`matchExpressions` supporting `In`, `NotIn`, `Exists`, `DoesNotExist`, `Gt`, `Lt` with value tags), and pod Tolerations (`NoSchedule`, `PreferNoSchedule`, `NoExecute` with `tolerationSeconds`). Offers one-click quick-load preset blueprints ("Standard Single-Node", "High-Throughput Dedicated", "Kernel-Isolated (gVisor)") and live Kubernetes JSON payload inspection for instant cluster deployment verification.
- 🚀 **Ad-Hoc Load Test Execution Dispatcher, Live Monitor & Abort Control (Web UI)**: Interactive load test execution dispatcher and real-time telemetry monitor in the React 19 web interface (`/runs`). Operators can dispatch on-demand test executions selecting active test suites, compiled artifacts filtered by platform architecture (`linux/amd64`, `linux/arm64`), scenario configurations, runner profiles with auto-filled resource specifications, and configurable execution timeout overrides (`activeDeadlineSeconds`). The live execution monitor features a visual phase transition stepper (`QUEUED` → `RUNNING` → `COMPLETED`/`FAILED`/`ABORTED`), 1-second wall-clock duration timer, real-time Server-Sent Events (`run_status_changed`), technical execution metadata, and indexed performance KPIs. Includes a high-contrast 'Abort Test' control with an accessible confirmation dialog explaining graceful Kubernetes pod teardown and SIGTERM log flushing, instant state transition to `ABORTED`, and cluster resource reclamation.
- ⏰ **Interactive Cron Builder & Scheduled Test Runs Management (Web UI)**: Full lifecycle management for automated load test scheduling in the React 19 web dashboard (`/schedules`). Features a human-readable **Interactive Cron Builder** with one-click presets ("Hourly", "Nightly", "Daily at midnight", "Every Monday morning", "Weekend soak"), instant 5-field syntax validation, natural language cadence preview, and next scheduled run calculation in UTC. Includes a comprehensive **Schedules Management Table** displaying schedule names, associated test suites, cron expressions, next trigger timestamps, and active/paused state badges. Provides seamless quick actions: on-demand immediate execution ("Run Now"), live pause/resume toggling that instantly synchronizes `spec.suspend` with native Kubernetes `CronJob`s via control plane REST APIs (`PUT /api/v1/schedules/{id}`), a slide-over **Execution History Drawer** inspecting historical runs spawned by specific schedules, and safe deletion dialogs.
- 📊 **Executive Test Run Summary, SLA Auditing & Recharts Performance Visualizations (Web UI)**: Comprehensive executive summary modal and dashboard in the React 19 web interface (`RunSummaryDashboard`). Features an instant **SLA Compliance Banner** with pass/fail evaluation, execution exit code, and wall-clock duration; primary KPI metric cards for total iterations, total requests, average throughput (TPS), error rate with dynamic normal/warning/critical color thresholds (<5% vs ≥5%), and latency percentile badges ($p_{50}, p_{90}, p_{95}, p_{99}$); and execution environment metadata (runner profiles, Kubernetes pod topology, cryptographic artifact SHA256 checksums, timestamps). Integrates accessible **Recharts Data Visualizations** with interactive tooltips, responsive mobile/tablet/desktop scaling, and synchronized semantic tabular data fallbacks: a **Latency Percentile Distribution Bar Chart** displaying $p_{50}$ to $p_{99}$ in milliseconds, a **Dual-Axis Throughput & Error Rate Correlation Chart** graphing TPS on the left Y-axis and error percentages on the right Y-axis across test duration, and a **10-Run Historical Regression Trend Chart** comparing latency percentiles across consecutive suite executions with current-run highlighting.
- 📜 **Virtualized ANSI Execution Log Viewer for run.log Streaming (Web UI)**: High-performance execution log viewer powered by `@tanstack/react-virtual` and zero-dependency ANSI escape sequence parsing (`VirtualizedLogViewer`). Seamlessly streams and renders 50,000+ line execution logs (`run.log`) with sub-millisecond virtual list scrolling and standard ANSI SGR formatting (bold, dim, italic, underline, standard 8 colors, high-intensity, 256 colors, and 24-bit TrueColor RGB). Features real-time log polling during active test runs, case-insensitive text search with total match count and bidirectional jump navigation, auto-scroll to bottom, line wrapping toggles, dark vs. high-contrast WCAG AAA terminal mode, one-click clipboard copying, and raw log downloading. Accessible directly within the Execution Runs dashboard (`/runs`), the Executive Run Summary, and the Live Execution Monitor.
- 🔍 **Interactive JSON Tree & Tabular Summary Report Inspector (Web UI)**: Advanced telemetry inspection tool for load test runs (`summary.json`). Features a dual-mode viewer seamlessly switching between a structured **Tabular Overview** and an interactive **Collapsible JSON Tree**. The tabular overview categorizes execution telemetry into Scenario Steps (with step duration, execution status, request counts, TPS, failure rates, and $p_{50}$–$p_{99}$ latency distributions), Aggregated Telemetry Metrics (type, count, min/avg/max values, percentiles), and SLA Threshold Evaluations (pass/fail status, actual vs. target constraints). The interactive JSON tree provides deep hierarchical inspection with collapsible nodes, recursive expand/collapse all, path-based search filtering with query highlighting, one-click JSON path copying (`foo.bar[0].baz`), and full payload clipboard export. Operators can trigger instant client-side RFC 4180 **CSV Exports** (with step, metric, or threshold breakdown) and formatted **JSON Downloads** directly within the Execution Runs dashboard (`/runs`), the Executive Run Summary modal, and the live execution inspector.
...
- 📚 **In-App Native Markdown Viewer for Control Plane Cookbook & Adoption Guide (Web UI)**: An accessible, full-featured native Markdown viewer embedded directly in the React 19 web interface (`/cookbook`), accessible via the primary navigation sidebar, tablet drawer, mobile bottom bar, and Dashboard quick links. Safely compiles GitHub Flavored Markdown (`docs/cookbook.md`) using `react-markdown` and `remark-gfm` with zero raw HTML injection (`dangerouslySetInnerHTML`). Features an auto-generated sticky Table of Contents (TOC) with real-time keyword search filtering, smooth anchor jump links, syntax-highlighted code blocks with one-click clipboard copying and visual/screen-reader confirmation, responsive tables, and GitHub-style callout alerts (`[!TIP]`, `[!NOTE]`, `[!WARNING]`, `[!CAUTION]`, `[!IMPORTANT]`). Bundled for 100% offline and air-gapped delivery in isolated Kubernetes environments, with dependencies isolated into an on-demand vendor chunk (`vendor-markdown`).
...
- 🚀 **Automated CI/CD & Multi-Arch Container Image Publishing**: End-to-end GitHub Actions pipelines validate pull requests and pushes to `main`. Automated workflows gate code changes through React 19 frontend linter checks (`pnpm lint`), Vitest unit/component tests (`pnpm test`), production asset builds (`pnpm build`), and Go BFF static analysis (`golangci-lint`) with race-detector testing (`go test -v -race ./cmd/bff/... ./internal/bff/...`). Docker pipelines build and publish multi-arch images (`linux/amd64`, `linux/arm64`) for the Go BFF (`ghcr.io/morphy76/vuhive-cloud/bff`) to GitHub Container Registry (GHCR), tagging `latest` on `main` and publishing cryptographically signed images via Sigstore Cosign on SemVer release tags.
