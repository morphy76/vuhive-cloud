...
- 🌐 **Modern React 19 Web Interface & PWA**: The official frontend (`web/`) built with React 19, Vite 6, TypeScript, and zero-runtime Tailwind CSS v4. Features an adaptive responsive shell with a collapsible sidebar for desktop ($\ge 1024\text{px}$), a touch-gesture slide-out drawer for tablets ($768\text{px}-1023\text{px}$), and a fixed bottom navigation bar for mobile ($<768\text{px}$). Includes dark/light theme switching, vendor-split asset chunking (`vendor-react`, `vendor-ui`), and zero-overhead delivery directly via the Go BFF's embedded `embed.FS` file server with immutable HTTP caching.
