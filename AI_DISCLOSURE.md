...
| [`.agents/rules/k8s-local-validation.md`](./.agents/rules/k8s-local-validation.md) | **Local Kubernetes Validation Safety (Rancher Desktop)**:<br>• Context verification locked to `rancher-desktop`.<br>• Isolated ephemeral namespaces (`vuhive-smoke-*`) with guaranteed post-validation teardown.<br>• BuildKit `--load` image containment and in-cluster probing. |
