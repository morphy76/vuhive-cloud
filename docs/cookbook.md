...
- **`SuiteSecret`**: An encrypted credential (API key, OAuth token, basic authentication secret) scoped to a `TestSuite`. Persisted encrypted at rest with AES-256-GCM and resolved dynamically into scenario configuration YAMLs via `${secrets.KEY}` placeholder syntax during runner initialization.
